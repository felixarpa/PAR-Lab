...
setenv size 32768
set merge_size_list = "1 2 4 8 16 32 64 128 256 512 1024 2048 4096"
setenv sort_size 32

set i = 1

set out = $PROG-$OMP_NUM_THREADS-merge_size.txt
rm -rf $out
rm -rf ./elapsed.txt
foreach merge_size ( $merge_size_list )
	echo $merge_size >> $out
...
foreach merge_size ( $merge_size_list )
	echo "hash_label at " $i " : " $merge_size >> ./hash_labels.txt
	set i = `echo $i + 1 | bc -l`
end

jgraph -P depth-omp.jgr > $PROG-$OMP_NUM_THREADS-merge_size.ps
rm -rf ./hash_labels.txt
